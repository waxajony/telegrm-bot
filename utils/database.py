...
class Database:
    ...
    def get_ads_all(self):

        ads = self.cursor.execute(
                f"SELECT id, ad_title, ad_text, ad_price, ad_images FROM ads;"
            )
        return ads.fetchall()

    def get_ads_one(self, ads_id):

        ads = self.cursor.execute(
                f"SELECT id, ad_title, ad_text, ad_price, ad_images,ad_phone FROM ads WHERE id=?;",
                (ads_id,)
            )
        return ads.fetchone()